...
import { P as PrivyEvents } from './events-context-c90o6r-H.js';
export { C as CallbackError } from './events-context-c90o6r-H.js';
...
interface PrivyInterface {
     ...
     * This will open the Privy Modal which will guide the user through this action.
     *
     * @param options.name Optional passkey name used when linking
     */
    linkPasskey: (options?: {
        name?: string;
    }) => void;
...
type LinkWithOAuthInput = {
    /**
     * The OAuth provider to link to the authenticated user.
     */
    provider: OAuthProviderID;
};
type UseLinkWithOAuth = {
    /**
     * Redirect to an OAuth provider to link that provider account to the
     * authenticated user without using Privy's UI.
     *
     * This hook must be mounted on the page that the OAuth provider redirects
     * back to so it can complete the link flow.
     *
     * @returns a Promise that resolves when the redirect begins, or rejects if
     * there was an error starting the OAuth link flow.
     */
    initOAuth: ({ provider }: LinkWithOAuthInput) => Promise<void>;
    /**
     * Indicates a headless OAuth flow is in progress.
     */
    loading: boolean;
    /**
     * The current state of the OAuth link flow.
     */
    state: OAuthFlowState;
};
/**
 * @experimental
 *
 * Use this hook to link an OAuth account to an authenticated user, without
 * using any Privy UIs. This mirrors {@link useLoginWithOAuth}, but enforces
 * link-specific guards.
 */
declare const useLinkWithOAuth: () => UseLinkWithOAuth;
...
type SendCodeToLinkEmail = {
    email: string;
};
type UseLinkEmail = {
    /**
     * Sends a one-time-passcode (OTP) to an email address to link it to the
     * authenticated user.
     *
     * @param opts options for sending the OTP
     * @param opts.email {string} the email address to link
     *
     * @returns a Promise that resolves if the OTP was successfully sent to the email,
     * or rejects if there was an error.
     */
    sendCode: ({ email }: SendCodeToLinkEmail) => Promise<void>;
    /**
     * Link the email to the authenticated user with a one-time-passcode (OTP)
     * that has been sent via {@link sendCode}.
     *
     * For a single OTP, you may call this method up to a maximum of 5 times, to handle
     * errors where the user incorrectly enters the OTP in your UIs.
     *
     * After 5 attempts of {@link linkWithCode}, the existing OTP is no longer valid, and you
     * must request a new one for the user via {@link sendCode}.
     *
     * @param opts {@link LoginWithCode} options for linking the email with an OTP
     * @param opts.code {string} the OTP provided by the user
     *
     * @returns a Promise resolving to the updated user, or rejects if there was an error.
     */
    linkWithCode: (o: LoginWithCode) => Promise<{
        user: User;
    }>;
    /**
     * The current state of the link email flow.
     */
    state: OtpFlowState;
};
/**
 * @experimental
 *
 * Hook to link an email account to an authenticated user, without UIs.
 *
 * @returns The methods for linking email
 */
declare const useLinkEmail: () => UseLinkEmail;
...
type SendCodeToLinkPhone = {
    phoneNumber: string;
};
type UseLinkPhone = {
    /**
     * Sends a one-time-passcode (OTP) via SMS to a phone number to link it to
     * the authenticated user.
     *
     * The phone number will ignore all non numerical values in the string except for the optional
     * '+' first character for country code specification. By default, the implicit phone number
     * country code is +1/US. To send to non-US numbers, prepend `+${countryCode}` to the input.
     *
     * @param opts options for sending the OTP
     * @param opts.phoneNumber {string} the phone number to link
     *
     * @returns a Promise that resolves if the OTP was successfully sent,
     * or rejects if there was an error.
     */
    sendCode: ({ phoneNumber }: SendCodeToLinkPhone) => Promise<void>;
    /**
     * Link the phone number to the authenticated user with a one-time-passcode (OTP)
     * that has been sent via {@link sendCode}.
     *
     * For a single OTP, you may call this method up to a maximum of 5 times, to handle
     * errors where the user incorrectly enters the OTP in your UIs.
     ...
     * must request a new one for the user via {@link sendCode}.
     *
     * @param opts {@link LoginWithCode} options for linking the phone with an OTP
     * @param opts.code {string} the OTP provided by the user
     *
     * @returns a Promise resolving to the updated user, or rejects if there was an error.
     */
    ...
    /**
     * The current state of the link phone flow.
     */
    state: OtpFlowState;
};
/**
 * @experimental
 *
 * Hook to link a phone number to an authenticated user, without UIs.
 *
 * @returns The methods for linking phone
 */
declare const useLinkPhone: () => UseLinkPhone;
